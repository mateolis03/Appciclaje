...
public class DetalleFinalizar extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detalle_finalizar);
        ...
        FirebaseDatabase.getInstance().getReference("solicitudes").child(id).setValue(map).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                    Toast.makeText(DetalleFinalizar.this, "Solicitud Finalizada", Toast.LENGTH_LONG).show();
